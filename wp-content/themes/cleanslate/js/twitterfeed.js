...
var twitterFeed = function(getTweetsURL, currentProfile, currentSN, noOfTweets) {
    
    $j(document).ready(function () {
        var displaylimit = noOfTweets;
        var twitterprofile = currentProfile;
    	var screenname = currentSN;
        ...
        $j.getJSON(tweetsRequestUrl, 
            function(feeds) {   
               // alert(feeds);
    			...
    			if (showtweetactions == true) {				
    				$j('.twitter-article').hover(function(){
    					$j(this).find('#twitter-actions').css({'display':'block', 'opacity':0, 'margin-top':-20});
    					$j(this).find('#twitter-actions').animate({'opacity':1, 'margin-top':0},200);
    				}, function() {
    					$j(this).find('#twitter-actions').animate({'opacity':0, 'margin-top':-20},120, function(){
    				...
    				$j('#twitter-actions a').click(function(){
