...
$j.fn.cycle = function(options, arg2) {
	...
	return this.each(function() {
		...
		opts.updateActivePagerLink = opts.updateActivePagerLink || $j.fn.cycle.updateActivePagerLink;
		...
		var $jcont = $j(this);
		var $jslides = opts.slideExpr ? $j(opts.slideExpr, this) : $jcont.children();
		var els = $jslides.get();
		...
		var opts2 = buildOptions($jcont, $jslides, els, opts, o);
		if (opts2 === false)
			return;
...
function triggerPause(cont, byHover, onPager) {
	var opts = $j(cont).data('cycle.opts');
	if (!opts)
		return;
...
function handleArguments(cont, options, arg2) {
	...
	function checkInstantResume(isPaused, arg2, cont) {
		if (!isPaused && arg2 === true) { // resume now!
			var options = $j(cont).data('cycle.opts');
			if (!options) {
...
function destroy(cont, opts) {
	if (opts.next)
		$j(opts.next).unbind(opts.prevNextEvent);
	if (opts.prev)
		$j(opts.prev).unbind(opts.prevNextEvent);
	
	if (opts.pager || opts.pagerAnchorBuilder)
		$j.each(opts.pagerAnchors || [], function() {
			this.unbind().remove();
		});
	opts.pagerAnchors = null;
	$j(cont).unbind('mouseenter.cycle mouseleave.cycle');
...
function buildOptions($jcont, $jslides, els, options, o) {
	...
	var meta = $j.isFunction($jcont.data) ? $jcont.data(opts.metaAttr) : null;
	if (meta)
		opts = $j.extend(opts, meta);
	if (opts.autostop)
	...
	if (opts.fit) {
		if (!opts.aspect) {
			...
			$jslides.each(function(){
				var $jslide = $j(this);
				var ratio = (opts.aspect === true) ? $jslide.width()/$jslide.height() : opts.aspect;
				if( opts.width && $jslide.width() != opts.width ) {
					$jslide.width( opts.width );
					$jslide.height( opts.width / ratio );
				}

				if( opts.height && $jslide.height() < opts.height ) {
					$jslide.height( opts.height );
					$jslide.width( opts.height * ratio );
				}
			});
		}
	}
		...
		$jslides.each(function(){
			...
			$jslide.css({
				"margin-left": opts.width ?
					((opts.width - $jslide.width()) / 2) + "px" :
					0,
				"margin-top": opts.height ?
					((opts.height - $jslide.height()) / 2) + "px" :
					0
			});
		});
	}
	...
	if (opts.center && !opts.fit && !opts.slideResize) {
		$jslides.each(function(){
			var $jslide = $j(this);
			$jslide.css({
				"margin-left": opts.width ? ((opts.width - $jslide.width()) / 2) + "px" : 0,
				"margin-top": opts.height ? ((opts.height - $jslide.height()) / 2) + "px" : 0
			});
		});
	}
		
	// stretch container
	var reshape = (opts.containerResize || opts.containerResizeHeight) && $jcont.innerHeight() < 1;
	if (reshape) { // do this only if container has no size http://tinyurl.com/da2oa9
		var maxw = 0, maxh = 0;
		for(var j=0; j < els.length; j++) {
			var $je = $j(els[j]), e = $je[0], w = $je.outerWidth(), h = $je.outerHeight();
			if (!w) w = e.offsetWidth || e.width || $je.attr('width');
			if (!h) h = e.offsetHeight || e.height || $je.attr('height');
			maxw = w > maxw ? w : maxw;
			maxh = h > maxh ? h : maxh;
		}
		...
		if (opts.containerResizeHeight && maxh > 0)
			$jcont.css({height:maxh+'px'});
	}

	var pauseFlag = false;  // https://github.com/malsup/cycle/issues/44
	if (opts.pause)
		$jcont.bind('mouseenter.cycle', function(){
			pauseFlag = true;
			this.cyclePause++;
	...
	$jslides.each(function() {
		...
		var $jel = $j(this);
		this.cycleH = (opts.fit && opts.height) ? opts.height : ($jel.height() || this.offsetHeight || this.height || $jel.attr('height') || 0);
		this.cycleW = (opts.fit && opts.width) ? opts.width : ($jel.width() || this.offsetWidth || this.width || $jel.attr('width') || 0);
	...
	if (opts.timeout) {
		...
		if (opts.speed.constructor == String)
			opts.speed = $j.fx.speeds[opts.speed] || parseInt(opts.speed,10);
		if (!opts.sync)
	...
	if (opts.next)
		$j(opts.next).bind(opts.prevNextEvent,function(){return advance(opts,1);});
	if (opts.prev)
		$j(opts.prev).bind(opts.prevNextEvent,function(){return advance(opts,0);});
	if (opts.pager || opts.pagerAnchorBuilder)
		buildPager(els,opts);
...
function supportMultiTransitions(opts) {
	var i, tx, txs = $j.fn.cycle.transitions;
...
function go(els, opts, manual, fwd) {
	...
	if ((manual || !p.cyclePause) && (opts.nextSlide != opts.currSlide)) {
		...
		if (opts.fxFn) // fx function provided?
			...
			$j.fn.cycle[opts.fx](curr, next, opts, after, fwd, manual && opts.fastOnEvent);
		else
			$j.fn.cycle.custom(curr, next, opts, after, fwd, manual && opts.fastOnEvent);
	}
	else {
		queueNext();
	}
...
$j.fn.cycle.updateActivePagerLink = function(pager, currSlide, clsName) {
   $j(pager).each(function() {
       $j(this).children().removeClass(clsName).eq(currSlide).addClass(clsName);
   });
};
...
$j.fn.cycle.next = function(opts) { advance(opts,1); };
$j.fn.cycle.prev = function(opts) { advance(opts,0);};
...
$j.fn.cycle.createPagerAnchor = function(i, el, $jp, els, opts) {
	var a;
	if ($j.isFunction(opts.pagerAnchorBuilder)) {
...
//		return false; // <== allow bubble
	...
	if ( /mouseenter|mouseover/i.test(opts.pagerEvent) ) {
		$ja.hover(pagerFn, function(){/* no-op */} );
	}
	else {
		$ja.bind(opts.pagerEvent, pagerFn);
	}
	
	if ( ! /^click/.test(opts.pagerEvent) && !opts.allowPagerClickBubble)
		$ja.bind('click.cycle', function(){return false;}); // suppress click
	
	var cont = opts.$jcont[0];
	var pauseFlag = false; // https://github.com/malsup/cycle/issues/44
...
$j.fn.cycle.custom = function(curr, next, opts, cb, fwd, speedOverride) {
	...
	var fn = function() {
		$jn.delay(animInDelay).animate(opts.animIn, speedIn, easeIn, function() {
			cb();
		});
	};
	$jl.delay(animOutDelay).animate(opts.animOut, speedOut, easeOut, function() {
		$jl.css(opts.cssAfter);
		if (!opts.sync) 
			fn();
	});
	if (opts.sync) fn();
...
// don't need.
//
$j.fn.cycle.transitions.none = function($jcont, $jslides, opts) {
...
$j.fn.cycle.transitions.fadeout = function($jcont, $jslides, opts) {
	$jslides.not(':eq('+opts.currSlide+')').css({ display: 'block', 'opacity': 1 });
...
$j.fn.cycle.transitions.shuffle = function($jcont, $jslides, opts) {
	var i, w = $jcont.css('overflow', 'visible').width();
	...
	opts.fxFn = function(curr, next, opts, cb, fwd) {
		...
		$jel.animate(opts.shuffle, opts.speedIn, opts.easeIn, function() {
			...
			if (fwd) {
				...
				$jel.css('z-index', parseInt(z,10)+1+count);
			}
	...
	$j.extend(opts.cssBefore, { display: 'block', opacity: 1, top: 0, left: 0 });
};
...
$j.fn.cycle.transitions.turnRight = function($jcont, $jslides, opts) {
	...
	$j.extend(opts.cssBefore, { top: 0, left: 0, width: 0 });
	opts.animIn.left = 0;
	opts.animOut.width = 0;
};
...
$j.fn.cycle.transitions.zoom = function($jcont, $jslides, opts) {
	opts.before.push(function(curr, next, opts) {
		...
		$j.extend(opts.animIn, { top: 0, left: 0, width: next.cycleW, height: next.cycleH });
		$j.extend(opts.animOut, { width: 0, height: 0, top: curr.cycleH/2, left: curr.cycleW/2 });
	});
	opts.cssFirst.top = 0;
	opts.cssFirst.left = 0;
	opts.cssBefore.width = 0;
...
$j.fn.cycle.transitions.toss = function($jcont, $jslides, opts) {
	...
	opts.before.push(function(curr, next, opts) {
		...
		if (!opts.animOut.left && !opts.animOut.top)
			$j.extend(opts.animOut, { left: w*2, top: -h/2, opacity: 0 });
		else
			opts.animOut.opacity = 0;
	});
	opts.cssBefore.left = 0;
